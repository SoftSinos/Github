...
uses uFuncao, dPrincipal, fPrincipal, uConfiguracao, uPessoa, uAgenda, fProcessando, fAgendaStarter, uMensagem, fAjuda;
...
procedure TFrmPainelDiario.BtnLimparClick(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmPainelDiario';
...
begin
  ...
  unPessoa.CarregaDbGrid(DtmPrincipal.Conexao, 2, 0, 0, '', 0, 'Funcionario', CdsProfissionais);
  ...
  begin
    ...
    begin
      if lDataEntrada = '' then
      begin
        StgGrid.Cells[0, I] := 'Horário' + #13 + #13 + FormatDateTime('DD/MM/YYYY', Now) + #13 + CdsHorarios.FieldByName('HrAgenda').AsString;
      end
      else
      begin
        StgGrid.Cells[0, I] := 'Horário' + #13 + #13 + lDataEntrada + #13 + CdsHorarios.FieldByName('HrAgenda').AsString;
      end;
      CdsHorarios.Next;
    end;
  end;
...
begin
  ...
  StgGrid.RowCount := StrToInt(unConfiguracao.NmValor) + 2 {Geração da linha de titulo};
  StgGrid.ColCount := unPessoa.NumeroPessoaTipoPessoa(DtmPrincipal.Conexao, DtmPrincipal.IdEmpresaLogada, 'Funcionario') + 1 {Geração da coluna de titulo};
...
begin
  ...
  //Pintão celula sim, celula não, coluna sim, coluna não.
