...
uses
  ...
  cxLookAndFeels, cxLookAndFeelPainters, cxContainer, cxEdit, Vcl.ComCtrls, dxCore, cxDateUtils, Data.DB, Datasnap.DBClient, Vcl.StdCtrls, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxCalendar, Vcl.Buttons, Vcl.ExtCtrls, Vcl.Grids, Vcl.DBGrids, Vcl.Mask, Vcl.Menus, cxButtons, cxCheckBox;
...
type
  TFrmNotaConsumoStarter = class(TFrmCadastro)
    ...
    procedure EdtVlPgtoChequeExit(Sender: TObject);
...
uses dPrincipal, uFuncao, uMensagem, uProcedure, uCarregaDados, fProcessando, uNota, uNotaItem, uNotaPagamento, fAjuda, fPessoaPesquisa, fProdutoPesquisa,
    fTipoPagamentoPesquisa, dRelatorios, uEmpresa, uPessoa, uServico, uProduto, uTipoPagamento, fPrincipal, fEmpresaPesquisa, uProdutoEstoque;
...
var
  ...
  unProdutoEstoque   : TProdutoEstoque;
...
begin
  ...
  DtmPrincipal.unLog.NmTabela   := 'NotaConsumoStarter';
  ...
  try
    ...
    begin
      lVlLancado := CdsTipoPagamento.FieldByName('VlTotal').AsCurrency;
    end;
    ...
    EdtVlRestante.Text := RetornaValorEmReal(lVlGeralGrid - lVlLancado);
  end;
end;
...
begin
  ...
  try
    unProdutoEstoque.Inicializar;
    unProdutoEstoque.BuscarEstoque(DtmPrincipal.Conexao, unProduto.IdProduto, Integer(CbbIdEmpresa.Properties.Items.Objects[CbbIdEmpresa.ItemIndex]));

    if unProdutoEstoque.QtdEstoque <= 0 then
    begin
      MsgAtencao(uMensagem.ProdutoInexistenteNoEstoque);
      abort;
    end;
...
begin
  ...
  try
    if LblSnFinalizada.Caption = 'CONSUMO FINALIZADO' then
    begin
      MsgInformacao(uMensagem.ConsumoFinalizado);
      abort;
    end;
    ...
    if StrToFloat(EdtVlPgtoCheque.Text) > 1 then
    begin
      unTipoPagamento.Inicializar;
      unTipoPagamento.BuscarNmTpPagamento(DtmPrincipal.Conexao, 'CHEQUE');

      if unTipoPagamento.IdTpPagamento > 0 then
      begin
        lIdTpPgtoCheque := unTipoPagamento.IdTpPagamento
      end;

      CdsTipoPagamento.Append;
      CdsTipoPagamento.FieldByName('IdTpPagamento') .AsInteger   := lIdTpPgtoCheque;
      CdsTipoPagamento.FieldByName('NmTpPagamento') .AsString    := 'CHEQUE';
      CdsTipoPagamento.FieldByName('VlPagamento')   .AsCurrency  := StrToFloat(EdtVlPgtoCheque.Text);
      CdsTipoPagamento.Post;
    end;
  finally
...
begin
  ...
  uProcedure.Registro(DtmPrincipal.IdEmpresaLogada, StrToInt(EdtIdRegistro.Text), 'NotaConsumoStarter', 'Anterior', CdsRetorno);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        unNotaItem.Inativar(DtmPrincipal.Conexao, CdsProduto.FieldByName('IdNoteItem').AsInteger, Integer(CbbIdEmpresa.Properties.Items.Objects[CbbIdEmpresa.ItemIndex]), StrToInt(EdtIdRegistro.Text));
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        unNotaItem.Inativar(DtmPrincipal.Conexao, CdsServico.FieldByName('IdNoteItem').AsInteger, Integer(CbbIdEmpresa.Properties.Items.Objects[CbbIdEmpresa.ItemIndex]), StrToInt(EdtIdRegistro.Text));
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        unNotaPagamento.Inativar(DtmPrincipal.Conexao, 0, Integer(CbbIdEmpresa.Properties.Items.Objects[CbbIdEmpresa.ItemIndex]), StrToInt(EdtIdRegistro.Text),  CdsTipoPagamento.FieldByName('IdTpPagamento').AsInteger);
...
begin
  ...
  try
    ...
    begin
      ...
      unProduto.BuscarNrEan(DtmPrincipal.Conexao, FrmProdutoPesquisa.CdsPesquisa.FieldByName('NrEan').AsString);
...
begin
  ...
  uProcedure.Registro(DtmPrincipal.IdEmpresaLogada, StrToInt(EdtIdRegistro.Text), 'NotaConsumoStarter', 'Proximo', CdsRetorno);
  if CdsRetorno.RecordCount > 0 then
  begin
    CopulaTela;
  end
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if MsgConfirmacao(uMensagem.DesejaFinalizarOConsumo) then
        begin
          unNota.Finalizar(DtmPrincipal.Conexao, unNota.IdNota, Integer(CbbIdEmpresa.Properties.Items.Objects[CbbIdEmpresa.ItemIndex]))
        end;

        MsgInformacao(uMensagem.ORegistroFoiSalvoComSucesso + #13 + 'CONSUMO: Nº: ' + EdtIdRegistro.Text);
...
begin
  ...
  unProdutoEstoque := TProdutoEstoque.Create;
end;
...
begin
  ...
  LblSnFinalizada.Caption         := 'CONSUMO ABERTO';
...
procedure TFrmNotaConsumoStarter.MkeNrCelular1Exit(Sender: TObject);
var
  Key: Char;
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
...
begin
  ...
  try
    ...
    while not CdsProduto.Eof do
    begin
      if CdsServico.FieldByName('IdNotaItem').AsInteger > 0 then
      begin
        unNotaItem.IdNotaItem       := CdsProduto.FieldByName('IdNotaItem').AsInteger;
      end
      else
      begin
...
begin
  ...
  try
    ...
    begin
      if CdsServico.FieldByName('IdNotaItem').AsInteger > 0 then
      begin
        unNotaItem.IdNotaItem       := CdsServico.FieldByName('IdNotaItem').AsInteger;
      end
      else
      begin
        unNotaItem.IdNotaItem       := 0;
...
begin
  ...
  try
    ...
    begin
      if CdsTipoPagamento.FieldByName('IdTpPagamento').AsInteger > 0 then
      begin
        unNotaPagamento.IdNotaPagamento  := CdsTipoPagamento.FieldByName('IdNotaPagamento').AsInteger;
      end
      else
      begin
...
begin
  ...
  try
    ...
    lVlRestante := lVlRestante + StrToCurr(RemoveMoeda(EdtVlPgtoCheque.Text));

    if lVlRestante < lVlGeral then
    begin
      EdtVlRestante.Text := RetornaValorEmReal((lVlGeral - lVlLancado) - lVlRestante);
    end
    else
    begin
...
procedure TFrmNotaConsumoStarter.EdtVlPgtoChequeExit(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmNotaConsumoStarter';
  DtmPrincipal.unLog.NmMetodo   := 'EdtVlPgtoChequeExit';
  DtmPrincipal.unLog.NmTabela   := 'NotaConsumoStarter';
  DtmPrincipal.unLog.GerarLog(DtmPrincipal.Conexao);

  try
    EdtVlPgtoCheque.Text := RemoveMoeda(RetornaValorEmReal(StrToCurr(EdtVlPgtoCheque.Text)));
  finally
    ValorRestante;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      lVlTroco              := lVlDinheiro - (lVlGeral - lVlLancado);
