...
uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics, Vcl.Imaging.jpeg, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, fCadastro, cxGraphics,
  cxControls, cxLookAndFeels, cxLookAndFeelPainters, cxContainer, cxEdit, Vcl.ComCtrls, dxCore, cxDateUtils, Data.DB, Datasnap.DBClient, Vcl.StdCtrls, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxCalendar, Vcl.Buttons, Vcl.ExtCtrls, Vcl.Menus, cxButtons, cxImage, cxDBEdit, cxCheckBox, cxMemo, Vcl.Grids, Vcl.DBGrids;
...
type
  TFrmProduto = class(TFrmCadastro)
    ...
    procedure DbgEmpresaDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
    procedure DbgEmpresaDblClick(Sender: TObject);
    procedure BtnAdiProdutoValorClick(Sender: TObject);
  private
...
uses dPrincipal, uFuncao, uMensagem, uProcedure, uCarregaDados, fProcessando, uProduto, fAjuda, fMarcaPesquisa, fGrupoProdutoPesquisa, fWebCam, uProdutoValor;

var
  unProduto        : TProduto;
  unProdutoValor   : TProdutoValor;
...
procedure TFrmProduto.BtnAdiProdutoValorClick(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmProduto';
  DtmPrincipal.unLog.NmMetodo   := 'BtnAdiProdutoValor';
  DtmPrincipal.unLog.NmTabela   := 'Produto';
  DtmPrincipal.unLog.GerarLog(DtmPrincipal.Conexao);
  ...
  try
    ...
    begin
      if CdsPesquisa.FieldByName('SnSelecao').AsBoolean then
      begin
        unProdutoValor.IdProdutoValor   := CdsPesquisa.FieldByName('IdProdutoValor').AsInteger;
        unProdutoValor.IdUsuario        := DtmPrincipal.IdUsuarioLogado;
        unProdutoValor.DtCadastro       := Now;
        unProdutoValor.VlCusto          := StrToFloat(MkeVlCusto.Text);
        unProdutoValor.VlVenda          := StrToFloat(MkeVlVenda.Text);

        if unProdutoValor.Salvar(DtmPrincipal.Conexao) then
        begin
          MsgInformacao(uMensagem.OValorDoProdutoFoiSalvoComSucesso);
        end;
      end
      else
      begin
    ...
    begin
      unProduto.CarregaDbGrid(DtmPrincipal.Conexao, 2, CdsPesquisa.FieldByName('IdProduto').AsInteger, DtmPrincipal.IdEmpresaLogada, '',  CdsPesquisa);
    end
    else
    begin
      unProduto.CarregaDbGrid(DtmPrincipal.Conexao, 2, CdsPesquisa.FieldByName('IdProduto').AsInteger, 0, '', CdsPesquisa);
    end;
    MkeVlCusto.Clear;
    MkeVlVenda.Clear;
  end;
...
begin
  ...
  try
    unProduto.CarregaDbGrid(DtmPrincipal.Conexao, 2, CdsRetorno.FieldByName('IdRetorno').AsInteger, 0, '', CdsCopula);
...
procedure TFrmProduto.DbgEmpresaDblClick(Sender: TObject);
begin
  if not CdsPesquisa.IsEmpty then
  begin
    if DbgEmpresa.Columns.Grid.SelectedField.FieldName = 'SnSelecao' then
    begin
      if CdsPesquisa.FieldByName('SnSelecao').AsBoolean then
      begin
        CdsPesquisa.Edit;
        CdsPesquisa.FieldByName('SnSelecao').AsBoolean := False;
        CdsPesquisa.Post;
      end
      else
      begin
        CdsPesquisa.Edit;
        CdsPesquisa.FieldByName('SnSelecao').AsBoolean := True;
        CdsPesquisa.Post;
      end;
    end;
  end;
...
procedure TFrmProduto.DbgEmpresaDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
  Check: Integer;
  R: TRect;
begin
...
begin
  ...
  unProdutoValor   := TProdutoValor.Create;
end;
