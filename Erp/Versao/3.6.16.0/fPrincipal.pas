...
type
  TFrmPrincipal = class(TForm)
    ...
    MenCadAgendaProfissional      : TMenuItem;
    ...
    procedure MenCadSuprimentoClick(Sender: TObject);
    procedure MenCadSangriaClick(Sender: TObject);
    procedure MenCadFecharCaixaClick(Sender: TObject);
    procedure MenCadAgendaStarterClick(Sender: TObject);
    procedure MenCadAgendaProfissionalClick(Sender: TObject);
   private
  ...
  public
    ...
    TasAgendaStarter      : TTabSheet;
    TasAgendaPremium      : TTabSheet;
    TasLog                : TTabSheet;
    TasPainel             : TTabSheet;
...
uses dRelatorios, uMensagem, uEmpresa, uProcedure, fCopiaSeguranca, fEmpresa, fLogin, fSplash, fSobre, fInformacoes, fProcessando, fConfiguracaoPesquisa, fLogPesquisa,
  ...
  fNotaEntradaEstoque, fDespesas, fBaixaParcelas, fPessoaRapido, fOperacaoFinanceira, fFechamentoCaixa, fAgendaPremium;
...
begin
  ...
  try
    ...
    FrmSplash.LblVersaoSplash.Caption := 'Aplicação: ' + VersaoExecutavel(DtmPrincipal.NmCaminhoExecutavel + DtmPrincipal.NmExecutavel) + #13 + 'Banco de Dados: ' + DtmPrincipal.NrVersaoBd;
...
procedure TFrmPrincipal.MenCadFecharCaixaClick(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmPrincipal';
  DtmPrincipal.unLog.NmMetodo   := 'MenCadFecharCaixaClick';
  DtmPrincipal.unLog.NmTabela   := 'Aplicacao';
  DtmPrincipal.unLog.GerarLog(DtmPrincipal.Conexao);
...
begin
  ...
  try
    ...
    begin
      ...
      TasConsultaNota.Caption     := 'Consultar Lançamentos';
...
begin
  ...
  try
    ...
    begin
      ...
      TasPainel.Caption     := 'Painel Diário';
...
procedure TFrmPrincipal.MenCadSangriaClick(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmPrincipal';
  DtmPrincipal.unLog.NmMetodo   := 'MenCadSangriaClick';
  DtmPrincipal.unLog.NmTabela   := 'Aplicacao';
  DtmPrincipal.unLog.GerarLog(DtmPrincipal.Conexao);

  try
    FrmOperacaoFinanceira := TFrmOperacaoFinanceira.Create(Application);
    FrmOperacaoFinanceira.Caption             := 'Lançar Sangria';
    FrmOperacaoFinanceira.GrbOperacao.Caption := 'Sangria';
    FrmOperacaoFinanceira.LblValor.Font.Color := clRed;
    FrmOperacaoFinanceira.LblValor.Caption    := 'Sangria:';
    FrmOperacaoFinanceira.ShowModal;
  finally
    FrmOperacaoFinanceira.Free;
  end;
end;
...
procedure TFrmPrincipal.MenCadSuprimentoClick(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmPrincipal';
  DtmPrincipal.unLog.NmMetodo   := 'MenCadSuprimentoClick';
  DtmPrincipal.unLog.NmTabela   := 'Aplicacao';
  DtmPrincipal.unLog.GerarLog(DtmPrincipal.Conexao);

  try
    FrmOperacaoFinanceira := TFrmOperacaoFinanceira.Create(Application);
    FrmOperacaoFinanceira.Caption             := 'Lançar Suprimento';
    FrmOperacaoFinanceira.GrbOperacao.Caption := 'Suprimento';
    FrmOperacaoFinanceira.LblValor.Font.Color := clBlue;
    FrmOperacaoFinanceira.LblValor.Caption    := 'Suprimento:';
    FrmOperacaoFinanceira.ShowModal;
  finally
    FrmOperacaoFinanceira.Free;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      TasConsumo.Caption     := 'Lançamento de Consumo';
...
procedure TFrmPrincipal.MenCadAgendaProfissionalClick(Sender: TObject);
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmPrincipal';
  DtmPrincipal.unLog.NmMetodo   := 'MenCadAgendaProfissionalClick';
  DtmPrincipal.unLog.NmTabela   := 'Aplicacao';
  DtmPrincipal.unLog.GerarLog(DtmPrincipal.Conexao);

  try
    if TasAgendaPremium = Nil then
    begin
      TasAgendaPremium             := TTabSheet.Create(PgcPrincipal);
      TasAgendaPremium.Caption     := 'Agenda do Profissional';
      TasAgendaPremium.PageControl := FrmPrincipal.PgcPrincipal;
    end
    else
    begin
      TasAgendaPremium.TabVisible  := True;
    end;

    PgcPrincipal.Visible    := True;
    PgcPrincipal.ActivePage := TasAgendaPremium;
    Application.CreateForm(TFrmAgendaPremium, FrmAgendaPremium);
    FrmAgendaPremium.Parent := FrmPrincipal.TasAgendaPremium;
    //FrmAgendaProfissional.CarregarTela('Geral');
    FrmAgendaPremium.Show;
  finally
...
begin
  ...
  try
    ...
    if TasAgendaStarter <> nil then
    begin
      PgcPrincipal.Visible := TasAgendaStarter.TabVisible;
      if TasAgendaStarter.TabVisible then
      begin
        Abort;
      end
      else
      begin
        TasAgendaStarter := nil;
      end;
    end;

    if TasAgendaPremium <> nil then
    begin
      PgcPrincipal.Visible := TasAgendaPremium.TabVisible;
      if TasAgendaPremium.TabVisible then
      begin
        Abort;
      end
      else
      begin
        TasAgendaPremium := nil;
      end;
    end;
...
begin
  ...
  Relatorios('Pessoas', 'Clientes', 'Relatórios Clientes', 'Clientes:');
end;
...
begin
  ...
  Relatorios('Financeiros', 'Financeiros', 'Relatórios Tipos de Pagamentos', 'Pagamentos:');
end;
...
begin
  ...
  Relatorios('Produtos', 'Produtos', 'Relatórios Produtos', 'Produtos:');
end;
...
begin
  ...
  Relatorios('Servicos', 'Servicos', 'Relatórios Serviços', 'Serviços:');
end;
...
begin
  ...
  try
    if TasAgendaStarter = Nil then
    begin
      TasAgendaStarter             := TTabSheet.Create(PgcPrincipal);
      TasAgendaStarter.Caption     := 'Agenda do Starter';
      TasAgendaStarter.PageControl := FrmPrincipal.PgcPrincipal;
    end
    else
    begin
      TasAgendaStarter.TabVisible  := True;
    end;

    PgcPrincipal.Visible    := True;
    PgcPrincipal.ActivePage := TasAgendaStarter;
    Application.CreateForm(TFrmAgendaStarter, FrmAgendaStarter);
    FrmAgendaStarter.Parent := FrmPrincipal.TasAgendaStarter;
