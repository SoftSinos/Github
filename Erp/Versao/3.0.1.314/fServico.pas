...
uses dPrincipal, uFuncao, uMensagem, uProcedure, uCarregaDados, fProcessando, uServico, fAjuda, fTipoServicoPesquisa, fPessoaPesquisa;
...
procedure TFrmServico.CarregarTela;
begin
  DtmPrincipal.unLog.NmOperacao := 'Entrou';
  DtmPrincipal.unLog.NmClasse   := 'TFrmServico';
  ...
  unCarregaDados.CarregaDados(DtmPrincipal.Conexao, 'Pessoa', 0, 'Profissional', '', '', CbbIdProfissional);
...
begin
  ...
  try
    unServico.CarregaDbGrid(DtmPrincipal.Conexao, True, 2, CdsRetorno.FieldByName('IdRetorno').AsInteger, '', CdsCopula);
