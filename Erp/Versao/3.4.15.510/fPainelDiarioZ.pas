...
begin
  ...
  begin
    ...
    StgGrid.ColCount := AtendimentoProfissionalDia(DtmPrincipal.Conexao, Now, 'Profissional') + 1{Geração da coluna de titulo};
  end
  else
  begin
    StgGrid.RowCount := AtendimentoProfissionalDia(DtmPrincipal.Conexao, lDataEntrada, 'Atendimento') + 1 {Geração da linha de titulo};
    StgGrid.ColCount := AtendimentoProfissionalDia(DtmPrincipal.Conexao, lDataEntrada, 'Profissional') + 1{Geração da coluna de titulo};
  end;
