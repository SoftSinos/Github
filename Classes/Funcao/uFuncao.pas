...
function DiaDaSemana(I: word; Tipo: Integer): String;
const
  DSemama: array [1 .. 7] of string = ('Domingo', 'Segunda-Feira', 'Terça-Feira', 'Quarta-Feira', 'Quinta-Feira', 'Sexta-Feira', 'Sábado');
begin
  if (Tipo = 0) then
  begin
...
function MsgInformacao(AMensagem: String): Boolean;
begin
  Application.MessageBox(Pchar(AMensagem), 'INFORMAÇÃO', MB_OK + MB_ICONINFORMATION);
  Result := True;
end;

function MsgAtencao(AMensagem: String): Boolean;
begin
  Application.MessageBox(Pchar(AMensagem), 'ATENÇÃO',  MB_OK + MB_ICONEXCLAMATION);
  Result := True;
end;
...
begin
  Result := False;
  if (Application.MessageBox(Pchar(AMensagem), 'CONFIRMAÇÃO', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = mrYes) then
  begin
    Result := True;
  end;
end;
